...
namespace Compiler
{
    ...
    public class Compiler
    {
        ...
        {
            // The ErrorReporter already writes error lines to the console when errors are reported.
            // Here we only print a concise final summary so users can quickly see overall result.
            if (Reporter == null)
            {
                WriteLine("[Summary]: No Reporter Detected. Final Summary Unavailable.");
                return;
            }

            if (Reporter.HasErrors)
            {
                WriteLine($"[Summary]: Compilation failed: {Reporter.ErrorCount} error(s) reported.");
            }
            else
            {
                WriteLine("[Summary]: Compilation successful!");
            }
        }
